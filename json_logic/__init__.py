...
operations = {
    ...
    "!=": lambda a, b: auto_unbox(list(np.logical_not(np.equal(np.array(to_list(a),dtype=object),b)))),
    "!==": lambda a, b: auto_unbox(list(np.logical_not(np.char.equal(check_type(to_list(a)),check_type(b)) & np.equal(np.array(to_list(a),dtype=object),b)))),
    ">": lambda a, b: auto_unbox(list(np.array(to_list(a),dtype=object) > b)),
    ">=": lambda a, b: auto_unbox(list(np.array(to_list(a),dtype=object) >= b)),
    "<": lambda a, b: auto_unbox(list(np.array(to_list(a),dtype=object) < b)),
    "<=": lambda a, b: auto_unbox(list(np.array(to_list(a),dtype=object) <= b)),
    "!": lambda a: auto_unbox(list(np.logical_not(np.array(to_list(a),dtype=object)))),
    "!!": bool,
